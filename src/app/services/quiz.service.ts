...
export class QuizService {
  ...
  private apiServer = "http://localhost:54282/api";
  httpOptions={
  ...
  getById(Course_id,Level_id): Observable<IQuestion> {
    return this.http.get<IQuestion>(this.apiServer + 'questions/GetByID/' + this.httpOptions);
  
  } 
  getcourses(courseid):Observable<Icourse[]>{
    return this.http.get<Icourse[]>(this.apiServer + "/Courses/" + courseid, this.httpOptions);
  }​​​​
  getAllCourses():Observable<Icourse[]>{
    return this.http.get<Icourse[]>(this.apiServer + "/Courses" );
  }​​​​
